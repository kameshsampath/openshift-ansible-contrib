...
add_node_openshift(){
  ...
  ansible new_nodes -m shell -a "curl -s https://raw.githubusercontent.com/kameshsampath/openshift-ansible-contrib/topology/single-master-multi-nodes/reference-architecture/azure-ansible/3.5/node.sh | bash -x" >/dev/null
...
add_master_openshift(){
  ...
  echo "Preparing the host..."
  ansible new_masters -m shell -a "curl -s https://raw.githubusercontent.com/kameshsampath/openshift-ansible-contrib/topology/single-master-multi-nodes/reference-architecture/azure-ansible/3.5/master.sh | bash -x"
